...
{
  std::string faceUri = ingress.face.getLocalUri().toString();
  std::string gotInterest = interest.getName().toUri();
  // std::cout << "forwarder got interest: " << gotInterest << std::endl;
  // std::cout << "faceUri = " << faceUri << std::endl;
  if (faceUri[0] =='a')
  {
    if(gotInterest[1] == 'n')
    {
      //discover neighour nodes
      // std::cout << "forwarder got interest: " << gotInterest << std::endl;   
      uint64_t n1 = gotInterest.find("-");
      // std::cout << "find = " << n1 << std::endl;
      std::string neiName = gotInterest.substr(n1+1, gotInterest.size()-n1-1);
      // std::cout << "neighour name: " << neiName << std::endl;
      const fib::Entry& fibEntryToCurrentReducer = m_fib.findLongestPrefixMatch(neiName);
      // std::cout << "after neighour name: " << neiName << std::endl;
      // std::cout <<  "fibEntry prefix: " << fibEntryToCurrentReducer.getPrefix().toUri() << std::endl;
      const fib::NextHopList& nextHopsToCurrentReducer = fibEntryToCurrentReducer.getNextHops();
      // std::cout <<  "next hops = : " << nextHopsToCurrentReducer.size()<< std::endl;
      uint64_t neiFaceCost = 0;
      for (const fib::NextHop& eachNextHop : nextHopsToCurrentReducer) 
      {
        neiFaceCost = eachNextHop.getCost();
        // std::cout << "face cost is: " << eachNextHop.getCost() << std::endl;
      }
      Name neiDataName(interest.getName());
      auto neiData = make_shared<Data>();
      neiData->setName(neiDataName);
      std::string rawData = boost::lexical_cast<std::string>(neiFaceCost);
      const uint8_t* p = reinterpret_cast<const uint8_t*>(rawData.c_str());
      auto buffer = make_shared< ::ndn::Buffer>(p, rawData.size());
      neiData->setContent(buffer);
      ingress.face.sendData(cref(*neiData));
      // ingress.face.sendData(cref(*neiData), ingress.endpoint);
    }
    else if (gotInterest[1] == 'f')
    {
      //get faceId from FIB.nexthop 
      // std::cout << "forwarder got interest: " << gotInterest << std::endl;
      uint64_t fp = gotInterest.find_first_of("-");
      uint64_t checkRejoin = gotInterest.find("/rejoin");
      std::string fTag;
      if (checkRejoin != std::string::npos) 
      {
        fTag = gotInterest.substr(fp+1, checkRejoin-fp-1);
      }
      else 
      {
        fTag = gotInterest.substr(fp+1);
      }
      // std::cout << "fTag: " << fTag << std::endl;
      const fib::Entry& currentFibEntry = m_fib.findLongestPrefixMatch(fTag);
      const fib::NextHopList& currentNextHops = currentFibEntry.getNextHops();
      uint64_t fibFaceId = 0;
      for (const fib::NextHop& eachNextHop : currentNextHops) 
      {
        fibFaceId = eachNextHop.getFace().getId();
        // std::cout << "fib face is: " << fibFaceId << std::endl;
      }
      Name fibDataName(interest.getName());
      auto fibData = make_shared<Data>();
      fibData->setName(fibDataName);
      std::string rawData = boost::lexical_cast<std::string>(fibFaceId);
      const uint8_t* p = reinterpret_cast<const uint8_t*>(rawData.c_str());
      auto buffer = make_shared< ::ndn::Buffer>(p, rawData.size());
      fibData->setContent(buffer);
      ingress.face.sendData(cref(*fibData));
      //std::cout<< "receive Fib" <<std::endl;

    }
    else if (gotInterest[1] == 'p' && gotInterest[2] == '-')
    {
      // get faceId stored in PIT
      // std::cout << "ask pit interest: " << gotInterest << std::endl;
      uint64_t p1 = gotInterest.find_first_of("-");
      std::string cName = gotInterest.substr(p1+1);
      // std::cout << "creat pit interest: " << cName << std::endl;
      shared_ptr<Name> checkPitName = make_shared<Name>(cName);
      shared_ptr<Interest> checkPitInterest = make_shared<Interest>(interest);
      checkPitInterest->setName(*checkPitName);
      //get Pit (or the ingress.face of current interest stored in PIT) and return
      shared_ptr<pit::Entry> searchPit = m_pit.find(cref(*checkPitInterest));
      // std::cout << "pit find flag: " << searchPit->getName() << std::endl;
      uint64_t searchPitFaceId = 0;
      for (const pit::InRecord& searchInRecord : searchPit->getInRecords()) 
      {
        Face* searchPitFace = &searchInRecord.getFace();
        searchPitFaceId = searchPitFace->getId();
        // std::cout << "pit find face: " << searchPitFaceId << std::endl;
      }
      Name pitDataName(interest.getName());
      auto pitData = make_shared<Data>();
      pitData->setName(pitDataName);
      std::string rawData = boost::lexical_cast<std::string>(searchPitFaceId);
      const uint8_t* p = reinterpret_cast<const uint8_t*>(rawData.c_str());
      auto buffer = make_shared< ::ndn::Buffer>(p, rawData.size());
      pitData->setContent(buffer);
      ingress.face.sendData(cref(*pitData));
